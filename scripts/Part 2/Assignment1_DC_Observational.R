...
###now checking the accuracy of the model. we now perform the detevtive work of checking if the our model can recover our hidden truth

summary(mdel)
# Check if the model is "happy"
# Check if the model is "happy"
par(mfrow = c(2, 2))
plot(mdel)

## Residual histogram (the Shape of error)
#which shows if our noise is actually centered at 0
...
yfit <- yfit * diff(hist(res, plot = FALSE)$mids[1:2]) * length(res)
lines(xfit, yfit, col = "red", lwd = 2)

#2 Residuals vs Individual predictors
#we need to check our residuals against each independent variable (eg here I only used outings however we can check all of them)to ensure the error is random for every factor
# Check if error is random across Social Outings
plot(student_data$social_outings, res,
     main = "Residuals vs Social Outings",
     xlab = "Number of Outings",
     ylab = "Residuals",
     pch = 19, col = "darkgrey")
